...
const Booking = () => {
    ...
    return (
        ...
        <div className=' bg-white pb-5'>
            ...
            <div >
            <button style={{backgroundColor:'black'}} class="btn" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasScrolling" aria-controls="offcanvasScrolling"><div className='icon'></div>
                                            <div className='icon'></div>
                                            <div className='icon'></div></button>
            <div class="offcanvas offcanvas-start" data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1" id="offcanvasScrolling" aria-labelledby="offcanvasScrollingLabel">
            <div class="offcanvas-header">
                <h3 class="offcanvas-title" id="offcanvasScrollingLabel">Top Takes</h3>
                <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
            </div>
            <div className="offcanvas-body">
               <Link className='text-decoration-none text-dark' to='/home'><h6>Home</h6></Link>
               <Link className='text-decoration-none text-dark' to='/gallery'><h6>Gallery</h6></Link>
               <Link className='text-decoration-none text-dark' to='/booking'><h6>Booking</h6></Link>
              

            </div>
            <h5 className="text-dark ms-2 mb-2 mt-2" style={{textShadow:"1px 1px rgb(83, 80, 80)"}}>{user.displayName}</h5>
                        {
                            user.email ? 
                            <div className="d-flex">
                              
                              <button style={{backgroundColor:'black',color: "white"}}className="rounded ms-2 my-2 border-0 py-2 px-3  py-sm-2 px-sm-3" onClick={logOut}>Sign Out</button>
                            </div>
                            :
                            <Link to="/login" style={{backgroundColor:'black'}} className='btn w-25 ms-2 my-3 text-white'>Login</Link>
                        }
         
            </div>
                <h1 style={{backgroundColor:'black'}} className="text-center text-white py-5">Packages</h1></div>
            </div>
            ...
            <div class="w-100 row row-cols-1 row-cols-md-3  content-card">
                {
                    photos.map(photo => <SingleBooking
                    key={photo.key}
                    photo={photo}
                    ></SingleBooking>)
                }
            ...
            <div class="w-100 row row-cols-1 row-cols-md-3   content-card">
                {
                    cinemas.map(cinema => <SingleBookingThree
                    key={cinema.key}
                    cinema={cinema}
                    ></SingleBookingThree>)
                }
            ...
            <div class="w-100 row row-cols-1 row-cols-md-3 content-card">
                {
                   addons.map(addon => <SingleBookTwo
